...
class Graphcls extends CustomPainter{
  ...
  void paint(Canvas canvas, Size size)
  {
    ...
    for(int i=0;i<l.length-1;i++) {
      Offset point = Offset(size.width/2, size.height-50-(l[i] + l[i + 1])/2);
      TextSpan span = TextSpan(style: const TextStyle(color: Colors.black87), text:"Zone "+((i+1).toString()));
    ...
    TextSpan span = TextSpan(style:const TextStyle(color: Colors.teal,fontWeight: FontWeight.bold,fontSize: 30), text:myData['title']);
    TextPainter tp = TextPainter(text: span, textDirection: TextDirection.ltr);
    tp.layout();
